...
@Component({
  selector: 'app-editar-elevador',
  templateUrl: './editar-elevador.component.html',
  styleUrls: ['./editar-elevador.component.css']
})
export class EditarElevadorComponent {
  listaCodEd: string[] = [];
  ...
  public editarElevador(): void {
    const codigo = this.myForm.get('codigo')?.value;

    /* 
    Campos opcionais
    Caso não sejam preenchidos, o valor é "" ou [] (no caso de pisosServidos)
    */
    const pisosServidos : number[] =[];

    if (this.listaNumeroPisosSelecionados !== null){
    ...
    this.elevadorService.editarElevador(codigo, pisosServidos, marca, modelo, numeroSerie, descricao);
  }
}
